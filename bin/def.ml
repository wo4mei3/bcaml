...
let check_valid_decl decl =
  let rec aux = function
    | (_, { ast = TDrecord (_, tyl, fields); _ }) :: rest ->
    ...
    | (_, { ast = TDvariant (_, tyl, fields); _ }) :: rest ->
        if List.for_all (fun (_, t) -> check_valid_ty tyl t) fields then
          aux rest
        else
          failwith
            "a type variable doesn't appear in the type parameter list is found"
    | (_, { ast = TDabbrev (_, tyl, ty); _ }) :: rest ->
...
let rec is_abbrev name = function
  | (_, { ast = TDabbrev (name', _, _); _ }) :: _ when name = name' -> true
...
and abbrev_found_in_decl name seen decl =
  let rec aux = function
    | (_, { ast = TDabbrev (n, _, ty); _ }) :: _ when n = name ->
...
let check_recursive_abbrev decl =
  let rec aux = function
    | (_, { ast = TDabbrev (name, _, _); _ }) :: rest ->
...
let rec is_def name = function
  | (_, { ast = TDrecord (name', _, _); _ }) :: _ when name = name' -> true
  | (_, { ast = TDvariant (name', _, _); _ }) :: _ when name = name' -> true
...
and def_found_in_decl name seen decl =
  let rec aux = function
    | (_, { ast = TDrecord (n, _, fields); _ }) :: _ when n = name ->
        let pair = (name, ref Checking) in
        List.iter
          (fun t -> def_found_in_ty decl (pair :: seen) t)
          (List.map snd fields);
        snd pair := Checked
    | (_, { ast = TDvariant (n, _, fields); _ }) :: _ when n = name ->
        let pair = (name, ref Checking) in
        List.iter
          (fun t -> def_found_in_ty decl (pair :: seen) t)
          (List.map snd fields);
...
let check_recursive_def decl =
  let rec aux = function
    | (_, { ast = TDrecord (name, _, _); _ }) :: rest ->
        def_found_in_decl name [] decl;
        aux rest
    | (_, { ast = TDvariant (name, _, _); _ }) :: rest ->
        def_found_in_decl name [] decl;
        aux rest
    | _ :: rest -> aux rest
    | [] -> ()
